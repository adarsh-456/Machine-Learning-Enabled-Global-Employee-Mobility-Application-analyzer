...
class DataIngestion:
    ...
    def export_data_into_feature_store(self)->DataFrame:
        ...
        try:
            if os.getenv(MONGODB_URL_KEY):
                ...
                local_data_file_path = self.data_ingestion_config.local_data_file_path
                logging.info(
                    f"{MONGODB_URL_KEY} is not set. Loading local data from: {local_data_file_path}"
                )
                if not os.path.exists(local_data_file_path):
                    raise FileNotFoundError(
                        f"Local data file not found at {local_data_file_path}. "
                        f"Set {MONGODB_URL_KEY} or provide a local CSV file."
                    )
                dataframe = pd.read_csv(local_data_file_path)
    ...
    def initiate_data_ingestion(self) ->DataIngestionArtifact:
        ...
        try:
            ...
            logging.info("Got the data for ingestion")
